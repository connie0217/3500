 ...
 * logged out.
 */
Ext.define('Sgai.util.SessionMonitor', {
  singleton: true,
  ...
  window: Ext.create('Ext.window.Window', {
    ...
    items: [{
      xtype: 'container',
      frame: true,
      html: "超过60分钟未操作，您的会话会超时。如果您的会话超时, 所有未保存的数据会丢失并且自动转到登录界面 </br></br>如果您想继续工作, 点击 '继续操作'  按钮.</br></br>"    
    },{
      xtype: 'label',
      text: ''
    }],
    buttons: [{
      text: '继续操作',
      handler: function() {
        Ext.TaskManager.stop(Sgai.util.SessionMonitor.countDownTask);
        Sgai.util.SessionMonitor.window.hide();
   ...
   */
  countDown: function() {
    this.window.down('label').update('您的会话将在 ' +  this.remaining  + ((this.remaining == 1) ? '秒' : '秒' + ' 后过期') );
